...
Animations in this library are implemented using Lottie animation library. You can get free animations files on [LottieFiles](https://lottiefiles.com/). You can find varieties of animation files on [https://lottiefiles.com](https://lottiefiles.com/). `*.json` file downloaded from _LottieFiles_ should be placed in android project. There are two ways to place the animation file (`*.json`).
