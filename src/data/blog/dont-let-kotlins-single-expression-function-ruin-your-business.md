...
If we call `greetGoodAfternoon().invoke()` or `greetGoodAfternoon()()` then it’ll be get executed (_Because you now know what happened_ 😄).
...
In my opinion, good practice to avoid mistakes will be a **_habit of mentioning return types explicitly_** ! Yes, if we mention it explicitly _(even if using a simple-expression function)_ then it’ll avoid mistakes then we’ll be sure about such things at compile time itself.
