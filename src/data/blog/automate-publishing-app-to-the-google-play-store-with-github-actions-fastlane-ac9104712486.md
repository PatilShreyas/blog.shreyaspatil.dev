...
*Finally, click**‘Apply’**→ Click**‘Invite User’*** Thus you’ve successfully set up Google play credentials. Keep that file with you and keep it safe.
...
Setting up Fastlane is so easy. [Ruby](https://www.ruby-lang.org/en/downloads/) should be preinstalled on the system. *(Alternatively, you can follow the process as described [here](https://docs.fastlane.tools/getting-started/android/setup/))** Create a `Gemfile` in the root directory of your Android project as below
...
*Then just run a command — `sudo gem install fastlane -NV`* Setup Fastlane using command —` fastlane init` and provide information with respect to your app.
